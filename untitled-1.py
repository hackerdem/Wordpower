        ...
        for i in range(1,number_of_rows):
            ...
            try:
                ...
                if len(word_dictionary)!=0:
                    if word_sheet.cell(row=i,column=1).value not in list(word_dictionary.keys()):
                        word_dictionary[word_sheet.cell(row=i,column=1).value]=[word_sheet.cell(row=i,column=1).value,word_sheet.cell(row=i,column=2).value,'0','1']
                else:
...
def append_read_data(r_file,word_list):
    for line in r_file:
        try:
            ...
            word_list[matching_pattern_in_line[0]]=matching_pattern_in_line
        except:
            pass 
...
def get_words():
    ...
    for i in range(0,40):
        ...
        selected_words.append(list(word_dictionary.values())[iterator])
...
def user_discard(fr,eng):
    #print(fr,eng)
    discard_words[fr]=word_dictionary[fr]
    del word_dictionary[fr]
def pause():
...
def delete_from_source():
    ...
    try:
        ...
        with open('data.txt','a+') as write_source_file:
            ...
            for key,i in word_dictionary.items():
                if key not in list(discard_words.keys()):
